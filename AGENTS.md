...
A bilingual (English / Hindi) offline-first reference web app holding nine duty-post instruction sets for a police deployment. A constable at a barricade scans a QR code on the laminated card at that post and lands directly on that post's instructions. No login. No PDF. Works with the phone in airplane mode after first load.
...
These override any competing instinct about what makes a good web app.
...
- **Hindi is a first-class language,** not a translation toggle nobody finds. Devanagari must render in a self-hosted Noto Sans Devanagari — never a system fallback.
